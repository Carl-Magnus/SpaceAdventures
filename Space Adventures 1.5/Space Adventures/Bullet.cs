...
namespace Space_Adventures
{
    class Bullet : MovingObject
    {
        ...
        public Bullet(Texture2D tex, Texture2D bulletTex, Vector2 startPos, Vector2 velocity, Rectangle hitBox, GameWindow window, bool shootRight, bool shootUp) : base(tex, bulletTex, startPos, hitBox, window)
        {
            this.tex = tex;
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            {
                direction = 1;
                startPos.X += startPos.X * (float)gameTime.ElapsedGameTime.TotalSeconds * (float)direction;
            }
            else if (!shootUp)
            {
                direction = -1;
            ...
            if (shootUp)
            {
                startPos.Y -= startPos.Y * (float)gameTime.ElapsedGameTime.TotalSeconds/* * (float)direction*/;
            }
        ...
        {
            //sb.Draw(tex, startPos, Color.White);            
            if (!shootRight && !shootUp)
            {
